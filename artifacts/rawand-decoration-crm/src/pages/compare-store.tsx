import { FileWarning, Search, Filter, RefreshCcw, ArrowDownUp } from "lucide-react";

export default function CompareStore() {
  return (
    <div className="">
      <div className="mb-4 flex items-center justify-between border-b border-gray-100 pb-2">
        <h1 className="text-xl font-normal text-gray-800">بەراوردکردنی کۆگا</h1>
        <div></div>
      </div>

      <div className="mb-3 flex flex-wrap items-center justify-between gap-3 flex-row">
        {/* Right side */}
        <div className="flex items-center gap-4 order-1 ml-auto">
          <div className="flex items-center gap-2">
            <select className="h-8 w-32 rounded-sm border border-gray-200 bg-white px-2 text-xs text-right outline-none">
              <option>هەموو</option>
            </select>
            <label className="text-xs font-bold text-gray-700">کۆگای یەکەم</label>
          </div>
          <span className="text-xs font-bold text-gray-400">بەرامبەر</span>
          <div className="flex items-center gap-2">
            <select className="h-8 w-32 rounded-sm border border-gray-200 bg-white px-2 text-xs text-right outline-none">
              <option>هەموو</option>
            </select>
            <label className="text-xs font-bold text-gray-700">کۆگای دووەم</label>
          </div>
        </div>
      ...
      <div className="overflow-x-auto rounded-sm border border-[#0f4c81] mt-4">
        <table className="w-full text-right text-xs">
          <thead className="bg-[#0f4c81] text-white">
            <tr>
              {["جیاوازی", "بڕ لە کۆگای دووەم", "بڕ لە کۆگای یەکەم", "بارکۆد", "ناوی کاڵا", "زنجیرە"].map((th, i) => (
