...
import { useCreateSale, useListAccounts, useListItems, getListSalesQueryKey } from "@workspace/api-client-react";
...
export default function NewSale() {
  ...
  // Basic state for the mockup
  const [accountId, setAccountId] = useState(1); // Default to a customer
  const [lines, setLines] = useState<{itemId: number, quantity: number, price: number, discount: number, name: string}[]>([]);
  
  // We'll just hardcode some items for selection to keep it simple in the UI
  const handleAddItem = () => {
    setLines([...lines, { itemId: Math.floor(Math.random() * 100) + 1, quantity: 1, price: 15000, discount: 0, name: "کاڵای نموونەیی" }]);
  };
  ...
  return (
    <div className="space-y-6">
      <PageHeader 
        title="پسوولەی فرۆشتنی نوێ" 
        breadcrumbs={[
          { name: "فرۆشتنەکان", href: "/sales" },
          { name: "پسوولەی نوێ" }
        ]}
        actions={
          <div className="flex gap-2">
            <Button variant="outline" asChild>
              <Link href="/sales"><ArrowRight className="w-4 h-4 me-2" /> گەڕانەوە</Link>
            </Button>
            <Button onClick={handleSave} disabled={lines.length === 0 || createSale.isPending} className="bg-blue-600 hover:bg-blue-700">
                      ...
                      <TableRow key={idx}>
                        ...
                        <TableCell>
                           <Input 
                            ...
                            className="text-end h-8 text-rose-500"
                            dir="ltr"
                          />
                        </TableCell>
          ...
          <Card>
            ...
            <CardContent className="space-y-4 pt-6">
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Input value="کڕیاری گشتی" readOnly className="bg-muted/50" />
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <Button variant="default" className="w-full">کاش</Button>
                  <Button variant="outline" className="w-full text-muted-foreground">قەرز</Button>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="pt-6">
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-bold text-2xl text-primary" dir="ltr">{formatCurrency(total)}</span>
                </div>
              </div>
            </CardContent>
          </Card>
