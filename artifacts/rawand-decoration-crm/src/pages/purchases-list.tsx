...
export default function PurchasesList() {
  ...
  const filteredPurchases = purchases.filter(p => 
    ...
    p.accountName.includes(searchTerm)
  );
  ...
  return (
    <div className="">
      <div className="mb-4 flex items-center justify-between border-b border-gray-100 pb-2">
        <div></div>
        <h1 className="text-xl font-normal text-gray-800">پسوولەکانی کڕین</h1>
      </div>
          ...
          <tbody>
            {isLoading ? (
              ...
              filteredPurchases.map((p) => (
                <tr key={p.id} className="border-b border-gray-100 hover:bg-gray-50 bg-white">
                  <td className="px-3 py-2" dir="ltr">{p.total} {p.currency}</td>
