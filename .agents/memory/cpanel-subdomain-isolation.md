...
**How to apply:** Before any CRM upload, confirm the exact target directory from cPanel's subdomain listing; do not assume a `public_html` child directory. Restore or verify the legacy root homepage separately, test both hostnames over HTTPS, and allow time for the subdomain certificate/vhost to become active before diagnosing a default-page response.